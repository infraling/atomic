...
import de.uni_jena.iaa.linktype.atomic.model.salt.editor.edit_policies.SPointingRelationConnectionEditPolicy;
 ...
 *
 */
public class SPointingRelationEditPart extends AbstractConnectionEditPart {
	
	private SPointingRelationAdapter adapter;
	private boolean labelled;

	public SPointingRelationEditPart() {
		super();
		adapter = new SPointingRelationAdapter();
	}
	...
	protected void createEditPolicies() {
		installEditPolicy(EditPolicy.DIRECT_EDIT_ROLE, new SPointingRelationDirectEditPolicy());
		installEditPolicy(EditPolicy.CONNECTION_ENDPOINTS_ROLE, new ConnectionEndpointEditPolicy());
		installEditPolicy(EditPolicy.CONNECTION_ROLE, new SPointingRelationConnectionEditPolicy());		
	}
	...
	protected List<EObject> getModelChildren() {
		...
		SPointingRelation model = (SPointingRelation) getModel();
	...
	protected IFigure createFigure() {
		return new SPointingRelationFigure(extractDisplayID());
	}
	
	private String extractDisplayID() {
		String sName = ((SPointingRelation) getModel()).getSName();
	...
	protected void refreshVisuals() { // TODO Refactor
		SPointingRelationFigure figure = (SPointingRelationFigure) getFigure();
	...
	public void activate() {
		if(!isActive()) {
			((SPointingRelation) getModel()).eAdapters().add(adapter);
	    }
		super.activate();
	}
	...
	public void deactivate() {
		if(isActive()) {
			((SPointingRelation) getModel()).eAdapters().remove(adapter);
		}
		super.deactivate();
	}
	...
	public class SPointingRelationAdapter implements Adapter {
	    ...
	    @Override public Notifier getTarget() {
	    	return (SPointingRelation) getModel();
	    }
