...
import de.uni_jena.iaa.linktype.atomic.model.salt.editor.edit_policies.SSpanComponentEditPolicy;
import de.uni_jena.iaa.linktype.atomic.model.salt.editor.edit_policies.SSpanDirectEditPolicy;
import de.uni_jena.iaa.linktype.atomic.model.salt.editor.edit_policies.SSpanGraphicalNodeEditPolicy;
 ...
 *
 */
public class SSpanEditPart extends AbstractGraphicalEditPart implements NodeEditPart {
	
	private SSpanAdapter adapter;
	
	public SSpanEditPart() {
		super();
		adapter = new SSpanAdapter();
	}
	...
	protected IFigure createFigure() {
		return new SSpanFigure(extractDisplayID());
	}
	...
	protected void createEditPolicies() {
		installEditPolicy(EditPolicy.DIRECT_EDIT_ROLE, new SSpanDirectEditPolicy());
		installEditPolicy(EditPolicy.GRAPHICAL_NODE_ROLE, new SSpanGraphicalNodeEditPolicy());
		installEditPolicy(EditPolicy.COMPONENT_ROLE, new SSpanComponentEditPolicy());
	}
	...
	protected void refreshVisuals() {
		SSpanFigure figure = (SSpanFigure) getFigure();
		SSpan model = (SSpan) getModel();
	...
	private String extractDisplayID() {
		String sName = ((SSpan) getModel()).getSName();
...
////					SProcessingAnnotation anno = ((SSpan)((SSpanEditPart) child).getModel()).getSProcessingAnnotation("ATOMIC_GEF_COORDINATES__Y");
////					int oldVal = (Integer) anno.getSValue();
////					anno.setSValue(oldVal - yDifference);
////					((SSpanEditPart) child).refresh();
////				}
////			}
...
//	}
	...
	private int calculateY(SSpan model, SSpanFigure figure, int height) {
	...
	private int calculateX(SSpan model, SSpanFigure figure, int width) {
...
//		return (((constraintsMap.lastEntry().getKey() + constraintsMap.lastEntry().getValue().width) -  constraintsMap.firstEntry().getKey()) /*/ constraintsMap.size()*/) - (width / 2);
	...
	public ConnectionAnchor getSourceConnectionAnchor(ConnectionEditPart connection) {
		return ((SSpanFigure) getFigure()).getConnectionAnchor();
	}
	...
	public void activate() {
		if (!isActive()) {
			((SSpan) getModel()).eAdapters().add(adapter);
		}
		super.activate();
	}
	...
	public void deactivate() {
		if (isActive()) {
			((SSpan) getModel()).eAdapters().remove(adapter);
		}
		super.deactivate();
	}
	
	/**
	 * Inner class SSpanAdapter 
	 */
	public class SSpanAdapter extends EContentAdapter {
